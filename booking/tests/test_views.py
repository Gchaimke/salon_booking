...
class TestBookingAdminViews(TestCase):
    ...
    def test_booking_settings_view(self):
        ...
        self.assertInHTML(
            '<input type="checkbox" name="booking_enable" class=" form-check-input" id="id_booking_enable" checked="">', mark_safe(force_str(response.content)))
        self.assertInHTML(
            '<input type="checkbox" name="disable_weekend" class=" form-check-input" id="id_disable_weekend">', mark_safe(force_str(response.content)))
    ...
    def test_booking_list_view(self):
        ...
        self.assertInHTML(
            f'<td>{DEMO_BOOKING_DATA["user_name"]}</td>', mark_safe(force_str(response.content)))
        self.assertInHTML(
            '<i class="far fa-pause-circle text-danger"></i>', mark_safe(force_str(response.content)))

    @skip("TODO: Fix the test")
    def test_approve_booking_view(self):
        response = self.cl.get(reverse("booking_update", kwargs={
                               "id": self.booking.pk, "type": "approved"}))
        self.assertEqual(response.status_code, 302)
        response = self.cl.get(reverse("booking_list"))
        self.assertEqual(response.status_code, 200)
        self.assertInHTML(
            '<i class="fas fa-check-square text-success"></i>', mark_safe(force_str(response.content)))

    @skip("TODO: Fix the test")
    def test_delete_booking_view(self):
        response = self.cl.get(reverse("booking_update", kwargs={
                               "id": self.booking.pk, "type": "delete"}))
        self.assertEqual(response.status_code, 302)
        response = self.cl.get(reverse("booking_list"))
        self.assertEqual(response.status_code, 200)
        self.assertInHTML(
            '<tbody></tbody>', mark_safe(force_str(response.content)))
